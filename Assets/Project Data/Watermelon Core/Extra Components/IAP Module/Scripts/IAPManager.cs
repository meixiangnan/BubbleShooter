...
namespace Watermelon
{
    ...
    public static class IAPManager
    {
        ...
        public static void Initialise(GameObject initObject, IAPSettings settings)
        {
            ...
            // Domain Reload disabled: static dictionaries can survive a previous Play session.
            ...
            {
                IAPItem item = items[i];
                if (item == null)
                    continue;

                if (productsTypeToProductLink.ContainsKey(item.ProductKeyType))
                {
                    Debug.LogWarning($"[IAP Manager]: Duplicate ProductKeyType '{item.ProductKeyType}' skipped.");
                    continue;
                }

                if (productsKeyToProductLink.ContainsKey(item.ID))
                {
                    Debug.LogWarning($"[IAP Manager]: Duplicate product ID '{item.ID}' skipped.");
                    continue;
                }
