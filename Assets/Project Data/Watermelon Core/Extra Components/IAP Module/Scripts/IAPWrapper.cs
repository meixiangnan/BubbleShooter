...
#endif
    {
        ...
        public override async void Initialise(IAPSettings settings)
        {
            ...
            try
            {
                ...
                catch (System.Exception servicesException)
                {
                    // Project not linked / offline — keep playing with fake store.
                    Debug.LogWarning($"[IAP Manager]: Unity Services unavailable ({servicesException.Message}). Using fake store.");
                }

                StandardPurchasingModule purchasingModule = StandardPurchasingModule.Instance();

                if (settings.UseTestMode || !servicesReady || Application.isEditor)
                {
                    purchasingModule.useFakeStoreAlways = true;
                    // Default = no on-screen Fake Store UI in Game view
                    purchasingModule.useFakeStoreUIMode = FakeStoreUIMode.Default;
                }
            ...
            {
                Debug.LogWarning($"[IAP Manager]: Init failed ({exception.Message}). Continuing without store.");
                // Mark module initialised anyway so Domain Reload / retry doesn't
                // re-register products and throw duplicate-key errors.
        ...
        public override void BuyProduct(ProductKeyType productKeyType)
        {
            ...
            if(item != null)
            {
                if (controller == null)
                {
                    IAPCanvas.ChangeLoadingMessage("Store unavailable");
                    IAPCanvas.HideLoadingPanel();
                    return;
                }
        ...
        public override ProductData GetProductData(ProductKeyType productKeyType)
        {
            ...
            {
                if (controller == null)
                    return new ProductData(item.ProductType);
